...
(注9：现在是2021年5月7日，我要开始看接下来的4节内容了。)

(注10：现在是2021年7月15日，我看我这篇博文最早是2020年1月5日写的，没想到时间不知不觉都过去超过1年半了，这7天的课程我还没看完。。。。。我的个亲娘啊。。。。。。现在正在看第6天的视频课程，开始做项目了，是仿写[CNode 社区](https://cnodejs.org)的一个留言板功能。不过随着时间的推移，我对于Node的熟悉程度肯定是要比1年半前更加熟悉的了。)
...
> 在Node中没有全局作用域，只有**模块作用域**。
>
> 模块作用域，简单理解就是**文件作用域**，超出这个文件的都不管用。
>
...
// require加载也不管用，require顶多是用来执行一下，
...
* 对于希望可以被其他模块访问的成员，我们就需要把这些**公开的成员**都挂载到`exports`接口对象中就可以了
...
[Node.js模块里exports与module.exports的区别?](https://www.zhihu.com/question/26621212/answer/58946177)
...
* npm5以后的安装包不需要加`--save`参数，他会自动保存依赖。
* 当你安装包的时候，会自动创建或者更新`package-lock.json`这个文件。
* `package-lock.json` 保存了`node_modules`整个包的依赖树信息(包括下载地址)。   当你把`node_moodules`文件夹删除后，如果你有`package-lock.json`这个文件，当你`npm i`重新下载`node_modules`时，下载速度比没有`package-lock.json`更快(直接解析`package-lock.json`这个文件就可以瞬间得到所有包的下载地址，会更快)
* 从文件来看，有一个`lock`称之为锁
  * 这个`lock`是用来锁定版本的。
  * 如果项目依赖了`1.1.1版本`，如果你重新`npm i`其实会下载最新版本，而不是1.1.1版本
  * 我们的目的就是希望可以锁住1.1.1这个版本，`package-lock.json`这个文件的另一个作用就是锁定版本号，防止第三方依赖自动升级新版(升级新版后可能包会出现问题。)
  * 举例：没有`package-lock.json`的时候，`package.json`里面显示`"jquery":"^1.11.1"`，前面的`"^"`表示1.11.1之上的任何版本都可以，所以实际你安装的jquery版本不一定是1.11.1。而在`package-lock.json`中它是真实保持了你下载的版本。

## path路径操作模块

> 参考文档：https://nodejs.org/dist/latest-v14.x/docs/api/path.html#path_path_dirname_path
...
* `path.parse`
  * 把一个路径转为对象
    ...
    * name：不包含后缀名的文件名
* `path.join`
  * 当你需要进行路径拼接的时候，推荐使用这个方法，避免自己手动写会拼错路径
* `path.isAbsolute`
  * 判断一个路径是否是绝对路径
...
- `__dirname`，是一个成员，可以用来**动态**获取**当前文件模块所属目录**的**绝对路径**
- `__filename`，可以用来**动态**获取当前文件的**绝对路径**（包含文件名）
...
> 提问：在vscode使用express搭建后端api时，使用bodyParser时不起作用，单词直接画了条线，req.body为空，bodyParser已经过时了吗？那现在应该怎么处理前端数据到后端?
>
> 回答：直接用express调用bodyParser的方法就可以了，bodyParser2019年就被弃用了。执行下面2行代码即可。
>
...
* 处理模版
* 配置开放静态资源
* 配置`art-template`模版引擎
* 配置简单路由,`/students`渲染静态页出来
* 路由设计 
* 提取路由模块
...
> 注意：MongoDB是不需要设计表的，但是我们实际在操作的时候，还是要设计的，毕竟我们在操作数据库的时候，总不能乱填数据吧。
...
* 而且数据表还支持约束(约束是为了保持数据的完整性)
  * 唯一的
  * 主键
  * 默认值
  * 非空 
* 非关系型数据库非常的灵活，有的非关系型数据库就是`key-value`键值对，没有表的概念。
...
* `db`
  * 查看当前操作的数据库
  * 默认是`test`，但是它是空的，所以你`show dbs`时是没有test这个选项的。只有你修改了test数据库，它才会显示。
...
第三方包：`mongoose`基于MongoDB官方做了一层封装，所以更加的简单，我们做项目用这个。
...
function add(x, y) {
    ...
    console.log(3);
    //到这里就结束了，不会等到前面的定时器，所以直接就返回了默认值undefined
}
...
//    ajax
//   这种情况必须通过：回调函数
//  什么是回调函数呢？你送你女朋友回家，你女朋友说她回家了以后给你打电话，她现在还没打，因为她还没到家，但是她到家了以后肯定会给你打的，她到家给你打电话这件事情就叫回调。
~~~
...
如果我讲到这种程度了，你还不太懂，说明一个问题，没有认知能力（哈哈哈，稍微打击了一下同学们）。

如何熟练到像定义一个变量一样来封装一个带有回调函数的方法。
...
add4(10, 20, function (ret) {
    console.log(ret);
    //我现在拿到这个结果之后可以做任何操作，这个就是封装函数的意义。
    //通常情况下封装函数都会出来一个结果，我们用这个结果来做一些事情。
})
...
基于原生XMLHTTPRequest封装get方法。(使用`hs -o`命令打开http服务器)
...
为了解决以上编码方式带来的问题(回调地狱嵌套)，所以在ES6中新增了一个API，叫`Promise`构造函数。

* Promise的英文就是承诺，保证的意思。(I Promise you)
* 读取文件，文件越大，读取速读就越慢，至于什么什么可以读取完成，你是说不准的。
* Promise你可以把它理解成一个容器，容器中存放了一个异步任务。
...
这里p1,p2,p3的代码高度重合，我们可以把代码进行封装一下。
